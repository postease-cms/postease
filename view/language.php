			...
			<div id="edit_panel" class="panel <?=($mode==2)?'panel-primary':'panel-default'?>">
				<?php if ($mode == 2):?>
					<div class="panel-heading clearfix" id="panel_edit">
						<i class="fa fa-pencil-square-o"></i> <?=TXT_LANGUAGE_LBL_NOWEDIT($this_name)?>
						<?php if ($this_id == 1):?>
							<label for="allow_delete">
								<input id="allow_delete" type="checkbox" data-target_name="<?=TXT_LANGUAGE_LBL_MULTILANGUAGE?>" disabled>
								<?=TXT_LANGUAGE_LBL_DISALLOWDELETE?></label>
						<?php else:?>
							<label for="allow_delete">
								<input id="allow_delete" type="checkbox" data-target_name="<?=TXT_LANGUAGE_LBL_MULTILANGUAGE?>">
								<?=TXT_LANGUAGE_LBL_ALLOWDELETE?></label>
						<?php endif?>
						<a href="./"><?=TXT_LANGUAGE_LBL_CANCELEDIT?></a>
					</div>
				<?php else:?>
					<div class="panel-heading" id="panel_new"><i class="fa fa-plus-circle"></i> <?=TXT_LANGUAGE_LBL_NEW?></div>
				<?php endif?>
				<div class="panel-body">
					<form id="language" role="form" action="./?mode=3" method="post">
						
						<div class="form-group col-md-3">
							<label class="control-label" for="name"><?=TXT_LANGUAGE_LBL_NAME?></label>
							<div><input class="form-control" type="text" id="name" name="name" value="<?=$this_name?>" placeholder="<?=TXT_LANGUAGE_PLH_NAME?>" required></div>
						</div>
						
						<div class="form-group col-md-3">
							<label class="control-label" for="slug"><?=TXT_LANGUAGE_LBL_SLUG?></label>
							...
							<div id="input_wrapper_slug" data-target_table="<?=$sortable_table?>" data-this_id="<?=$this_id?>">
								<input class="form-control needValidation" data-valid_type="slug" type="text" id="slug" name="slug" value="<?=$this_slug?>" placeholder="<?=TXT_LANGUAGE_PLH_SLUG?>" required>
							</div>
						</div>
						...
						<div class="form-group col-md-2">
							<label class="control-label" for="status_01"><?=TXT_LANGUAGE_LBL_STATUS?></label><br>
						...
						<div class="form-group col-md-1">
							...
							<br>
							<input class="btn btn-primary" type="submit" id="do_update" name="<?=$submit_name?>" value="<?=TXT_LANGUAGE_BTN_UPDATE?>">
							<span class="spinner hidden"><i class="fa fa-spinner fa-pulse"></i></span>
							<input class="btn btn-danger" type="button" id="do_delete" name="do_delete" value="<?=TXT_LANGUAGE_BTN_DELETE?>">
						</div>
					</form>
				</div>
			</div>
			...
			<?php if (count($records)):?>
				...
				<div class="panel panel-default registerdList">
					<div class="panel-heading">
						<?=TXT_LANGUAGE_LBL_LISTLANGUAGE?> <span id="amount" class="badge"><?=$record_counter?></span>
						<span><a href="./?view_page=cover&amp;entity_code=3"><i class="fa fa-arrow-circle-right" aria-hidden="true"></i> <?=TXT_COVER_LBL_TOCOVER?></a></span>
					</div>
					<div class="panel-body">
						<table id="languages" class="table table-hover">
							<thead>
							<tr>
								<th><?=TXT_LANGUAGE_THD_ID?></th>
								<th><?=TXT_LANGUAGE_THD_NAME?></th>
								<th><?=TXT_LANGUAGE_THD_SLUG?></th>
								<th><?=TXT_LANGUAGE_THD_STATUS?></th>
								<th></th>
							</tr>
							</thead>
							...
							<?php foreach ($records as $key => $row):?>
								<tr id="special_cat_<?=$key?>" class="<?=($key==$this_id)?'active':''?>">
									...
									<td data-status="<?=$row['status']?>">
										<?php if ($row['status'] == 1):?>
											<label class="label label-primary"><?=TXT_LANGUAGE_LBL_DISPLAY?></label>
										<?php else:?>
											<label class="label label-danger"><?=TXT_LANGUAGE_LBL_UNDISPLAY?></label>
										<?php endif?>
									</td>
									<td>
										<?php if ($mode != 2):?>
											<a class="btn btn-default btn-xs" href="./?mode=2&amp;id=<?=$row['id']?>"><?=TXT_LANGUAGE_LBL_EDIT?></a>
										<?php else:?>
											<a class="btn btn-default btn-xs" disabled><?=TXT_LANGUAGE_LBL_EDIT?></a>
										<?php endif?>
									</td>
								</tr>
							<?php endforeach?>
