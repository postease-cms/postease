...
(
		...
		'remote_address_allowed'          => '接続を許可する接続元IPアドレスを指定します。複数指定する場合はカンマ（,）で区切ります。指定しない場合はすべてのIPアドレスから接続が可能になります。',
		'origin_allowed'                  => '接続を許可するドメインを指定します。プロトコル（http:// or https://）から指定してください。複数指定する場合はカンマ（,）で区切ります。指定しない場合はすべてのドメインから接続が可能になります。',
