 ...
 */
function get_post($target, $config = array(), $recursive = true)
{
	return pesf_get_post($target, $config, $recursive);
}
function pesf_get_post($target, $config = array(), $recursive = true)
{
	global $database;
	global $pdo;
	global $table_prefix;
			...
			if ($recursive)
			{
		        ...
		        WHERE pc.value = :BASE_ID AND ci.type = 'relation'
				 ";
					...
					unset($read_ids);
					foreach ($relation_ids as $key => $target_id)
					{
						$post_relations['relations'][$key] = pesf_get_post($target_id, array(), false);
					}
				}
				catch(PDOException $e)
				{
			...
			try {
				...
				while ($record = $read_post_custom -> fetch(PDO::FETCH_ASSOC))
				{
					...
					if ($recursive)
					{
						...
						{
							$record['value'] = pesf_get_post($record['value'], array(), false);
...
{
	return pesf_get_comments($post_id, $config);
}
...
{
	return pesf_get_archives($config);
}
...
{
	return pesf_get_categories($config);
}
...
{
	return pesf_get_posttypes($config);
}
...
{
	return pesf_get_languages($config);
}
...
{
	return pesf_get_image_frames($type);
}
...
{
	return pesf_get_comment_items($post_id, $config);
}
...
{
	return pesf_get_contact_items($config, $with_hidden);
}
function pesf_get_contact_items($config = array(), $with_hidden = false)
{
	global $database;
	global $pdo;
	global $table_prefix;
...
{
	return pesf_reset_items($items, $targets);
}
 ...
 */
function type_items($items, $targets = array())
{
	return pesf_type_items($items, $targets);
}
 ...
 * @return string $html
 */
function html_form($items, $config = array(), $values = array(), $placeholders = array())
{
	return pesf_html_form($items, $config, $values, $placeholders);
}
 ...
 */
function combo_comment($post_id, $config_get = array(), $targets_reset = array(), $targets_type = array(), $config_html = array(), $values_html = array(), $placeholders_html = array())
{
	return pesf_combo_comment($post_id, $config_get, $targets_reset, $targets_type, $config_html, $values_html, $placeholders_html);
}
 ...
 * @return html | boolean(false)
 */
function combo_contact($config_get = array(), $targets_reset = array(), $targets_type = array(), $config_html = array(), $values_html = array(), $placeholders_html = array())
{
	return pesf_combo_contact($config_get, $targets_reset, $targets_type, $config_html, $values_html, $placeholders_html);
}
 ...
 */
function set_comment($postdata)
{
	return pesf_set_comment($postdata);
}
 ...
 */
function set_contact($postdata)
{
	return pesf_set_contact($postdata);
}
...
{
	return pesf_get_contact($contact_id, $config);
}
...
{
	return pesf_reset_contact($contact, $targets);
}
...
{
	return pesf_send_mail($config, $contact);
}
...
{
	return pesf_send_mail_extend($config_mail, $config_smtp, $contact);
}
 ...
 * @return boolean
 */
function combo_mail($postdata, $config_mail, $config_get = array(), $targets_reset = array())
{
	return pesf_combo_mail($postdata, $config_mail, $config_get, $targets_reset);
}
 ...
 * @return boolean
 */
function combo_mail_extend($postdata, $config_mail, $config_smtp, $config_get = array(), $targets_reset = array())
{
	return pesf_combo_mail_extend($postdata, $config_mail, $config_smtp, $config_get, $targets_reset);
}
 ...
 * [@param integer $send_target [ 1:admin / 2:customer / 3:both ]
 * @return bool
 */
function submit_form_send_mail ($contact_target, $form_data, $reset_items = array(), $mail_config = array(), $send_target = 3)
{
	return pesf_submit_form_send_mail ($contact_target, $form_data, $reset_items, $mail_config, $send_target);
}
function pesf_submit_form_send_mail ($contact_target, $form_data, $reset_items = array(), $mail_config = array(), $send_target = 3)
{
	global $database;
	global $pdo;
	global $table_prefix;
	...
	$mail_to      = (! empty($form_data['email']))            ? $form_data['email']            : '';
	$postdata     = $contact_target + $form_data;
	 ...
	 */
	$config_smtp = array();
	$sql = "SELECT item, value FROM {$table_prefix}metadata WHERE entity_code = 2 AND entity_id = {$posttype_id} AND item IN('use_smtp', 'host', 'user_name', 'password', 'port')";
	$read_records = $pdo -> prepare($sql);
	$read_records -> execute();
	while ($record = $read_records -> fetch(PDO::FETCH_ASSOC))
	{
	 ...
	 */
	$config_admin = array();
	$sql = "SELECT item, value FROM {$table_prefix}metadata WHERE entity_code = 2 AND entity_id = '{$posttype_id}' AND item LIKE '%admin%'";
	$read_records = $pdo -> prepare($sql);
	$read_records -> execute();
	while ($record = $read_records -> fetch(PDO::FETCH_ASSOC))
	{
	 ...
	 */
	if (! empty($mail_config['admin']))
	{
		foreach ($mail_config['admin'] as $key => $value)
		{
	...
	$sql = "SELECT item, value FROM {$table_prefix}metadata WHERE entity_code = 2 AND entity_id = '{$posttype_id}' AND item LIKE '%customer%'";
	$read_records = $pdo -> prepare($sql);
	$read_records -> execute();
	while ($record = $read_records -> fetch(PDO::FETCH_ASSOC))
	{
	 ...
	 */
	if (! empty($mail_config['customer']))
	{
		foreach ($mail_config['customer'] as $key => $value)
		{
	...
	if (! $new_contact_id = set_contact($postdata))
	{
		return false;
	}
	if (empty($contact = get_contact($new_contact_id, $contact_target)))
	{
		return false;
	}
	 ...
	 * Send Mail Admin
	 */
	if ($send_target == 1 || $send_target == 3)
	{
		if (! empty($config_admin) && $config_admin['use_auto_reply_admin'])
		{
			$contact = ($config_admin['use_input_values_admin']) ? $contact : array();
			if ($config_smtp['use_smtp'])
			{
				if (false === send_mail_extend($config_admin, $config_smtp, $contact)) return false;
			}
			else {
				if (false === send_mail($config_admin, $contact)) return false;
			}
		}
	}
	 ...
	 * Send Mail Customer
	 */
	if ($send_target == 2 || $send_target == 3)
	{
		if (! empty($config_customer) && $config_customer['use_auto_reply_customer'])
		{
			$config_customer['to'] = $mail_to;
			$contact = ($config_customer['use_input_values_customer']) ? $contact : array();
			if ($config_smtp['use_smtp'])
			{
				if (false === send_mail_extend($config_customer, $config_smtp, $contact)) return false;
			}
			else {
				if (false === send_mail($config_customer, $contact)) return false;
			}
		}
	}
...
{
	return pesf_set_counter($post_id, $type);
}
